...
const NotFound = () => {
  return (
    // Fullscreen flex container centering page content on a dark slate color scheme
    <div className="flex flex-col items-center justify-center min-h-[70vh] text-slate-100 px-4 select-none relative">
      
      {/* Decorative gradient glow orb in background */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-gradient-to-tr from-blue-600/10 via-purple-600/10 to-indigo-600/5 rounded-full blur-3xl pointer-events-none"></div>

      {/* Main glassmorphic container box */}
      <div className="relative z-10 text-center max-w-lg p-8 rounded-3xl bg-slate-900/60 border border-slate-800 backdrop-blur-md shadow-2xl">
        
        {/* Glow indicator icon header */}
        <div className="mx-auto w-16 h-16 flex items-center justify-center bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-2xl mb-6 shadow-inner animate-bounce">
          <ShieldAlert className="w-8 h-8" />
        </div>

        {/* Large 404 Error Header with blue-to-purple glowing gradient */}
        <h1 className="text-7xl md:text-8xl font-black tracking-tight bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent drop-shadow-lg animate-pulse">
          404
        </h1>

        {/* Text warning label */}
        <h3 className="text-lg md:text-xl font-bold text-slate-200 mt-4 tracking-wide">
