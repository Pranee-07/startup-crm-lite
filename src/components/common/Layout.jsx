...
import { Outlet, useLocation } from 'react-router-dom'; // Import Outlet for nested route rendering, and useLocation to retrieve active route info
  ...
  return (
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-slate-950 text-slate-100 font-sans">
      ...
      <div className="flex flex-col flex-1 h-full overflow-hidden">
        ...
        <header className="flex items-center justify-between h-20 px-6 bg-slate-900 border-b border-slate-800 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="p-2 -ml-2 rounded-lg text-slate-400 hover:text-slate-100 hover:bg-slate-800 lg:hidden focus:outline-none transition-colors"
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden md:block w-64">
              <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <Search className="w-4 h-4 text-slate-400" /> {/* Search Icon indicator */}
              </span>
              <input 
                type="text" 
                placeholder="Search CRM..." // Placeholder prompt
                className="w-full pl-9 pr-4 py-2 text-xs bg-slate-950 border border-slate-800 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-slate-200 placeholder-slate-500 transition-all"
              />
            </div>
            ...
            <button className="relative p-2 rounded-xl bg-slate-950 border border-slate-800 text-slate-400 hover:text-slate-100 hover:bg-slate-800 transition-colors cursor-pointer">
